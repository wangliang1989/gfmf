...
if (flag /= 0) then
    flag = 1
    write(8,*) "Unable to open file ", filename
    close(unit=50)
    return
end if
...
if (flag /= 0) then
    flag = 2
    write(8,*) "Error in reading SAC header ", filename
    close(unit=50)
    return
end if
...
if ((head%nvhdr>6) .or. (head%nvhdr<0)) then
    ...
    if ((head%nvhdr>6) .or. (head%nvhdr<0)) then
        flag = 6
        write(8,*) filename, " not in sac format ", head%nvhdr
        close(unit=50)
        return
    endif
end if
...
if (flag /= 0)then
    flag = 3
    write(8,*) "Not enough memory for ", filename
    return
end if
...
if (flag /= 0) then
   flag = 4
   write(8,*) "Error in reading SAC data", filename
   return
end if
close(unit=50)
...
if (flag /= 0) then
    flag = 5
    write(8,*) "Error in writing SAC file ", filename
    close(unit=50)
    return
end if
...
select case(tmark)
    ...
    case default
        flag = 7
        write(8,*) "illegal time mark ", tmark
        close(unit=50)
        return
end select
...
if (tref == -12345.) then
    flag = 8
    write(8,*) "Time mark undefined in ", filename
    close(unit=50)
    return
end if
